...
class Migrator:
    ...
    def _get_transformations(cls):
        return [c for c in DatabaseTransformation.__subclasses__() if not isabstract(c)]
    ...
    def migrate_db_to_version(self, db_handle, to_version):
        ...
        if relevant_transformations:
            logging.getLogger().info(f"Will run DB migration from {db_handle.get_local_db_version().value} to {to_version}")
